...
| Agent Spawning | **8.5** | `cli/tools.ts` + `cli/commands/council.ts` | ✅ CLI tool + `/fleet` `/council fleet` | maxRounds, background, worktreeIsolation wired. `/fleet <task>` launches parallel agents; `/loop [--max=N]` runs autonomous iterations; confidence-gated escalation on low-confidence output. Agent manifests in `.dantecode/agents/`. (LANE4 2026-03-21) |
...
| Production Maturity | **9.0** | CI/CD + release scripts + auto-update + feature-flags | ✅ fully shipped | CI matrix (Node 18/20/22, format/typecheck/lint/test/anti-stub/publish dry-run). PR review workflow with PDSE annotations. Non-blocking auto-update check on startup. Feature maturity registry (stable/beta/experimental). `/status` command. PowerShell + bash installers. (LANE8 2026-03-21) |
| Council System | **8.0** | `core/council/` + `cli/commands/council.ts` | ✅ CLI (`dantecode council fleet`) | CouncilOrchestrator + DanteCodeAdapter wired. `council fleet` subcommand orchestrates YAML-manifest agents in parallel worktrees. (LANE4 2026-03-21) |
...
**Overall post-Wave2 score: ~8.9** (Wave 2 complete; Agent Spawning 7.4→8.5, Production Maturity 7.5→9.0, Council 6.5→8.0; 2026-03-21)
