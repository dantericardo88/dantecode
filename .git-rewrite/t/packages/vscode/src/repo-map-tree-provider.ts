...
export class RepoMapTreeDataProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
  ...
  async getChildren(element?: vscode.TreeItem): Promise<vscode.TreeItem[]> {
    ...
    // Load repo map off the main thread to avoid blocking the VS Code UI
    const repoMap = await new Promise<ReturnType<typeof generateRepoMap>>((resolve) => {
      setImmediate(() => resolve(generateRepoMap(this.projectRoot, { maxFiles: 500 })));
    });
