...
export class IssueToPRPipeline {
   ...
   */
  async run(issue: GitHubIssueInfo, agentExecutor: AgentExecutor): Promise<IssueToPRResult> {
    ...
    try {
      ...
      // Step 6: Create PR
      this.emitProgress("creating_pr", `Creating pull request for issue #${issue.number}`);
      const prResult = await this.createPullRequest(branchName, issue, verificationOutput);

      // Step 7: Comment on issue
      this.emitProgress("commenting", `Commenting on issue #${issue.number} with PR link`);
   ...
   */
  async verify(worktreeDir: string): Promise<{ passed: boolean; output: string }> {
  ...
  async createPullRequest(
    ...
    if (verificationOutput) {
      bodyLines.push(`### Verification`, "```", verificationOutput.slice(0, 2000), "```", "");
    }
    ...
    const response = (await this.githubAPI("POST", `/repos/${this.config.repository}/pulls`, {
      title,
      body,
      head: branchName,
   ...
   */
  async commentOnIssue(issueNumber: number, prUrl: string, success: boolean): Promise<void> {
   ...
   */
  async cleanupWorktree(worktreeDir: string, branchName: string): Promise<void> {
