...
import { mkdir, copyFile, readdir, stat, symlink, writeFile } from "node:fs/promises";
...
async function resolveSource(source: string, projectRoot: string): Promise<string> {
  ...
  if (source.startsWith("http://") || source.startsWith("https://")) {
    ...
    execFileSync("curl", ["-sL", source, "-o", tarball], { stdio: "pipe" });
...
export async function installSkill(
  ...
  try {
    resolvedPath = await resolveSource(options.source, projectRoot);
  } catch (err: unknown) {
