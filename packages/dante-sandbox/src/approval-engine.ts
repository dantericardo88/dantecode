...
export class ApprovalEngine {
  ...
  get allowRules(): RegExp[] { return [...this._allowRules]; }
  get denyRules(): RegExp[] { return [...this._denyRules]; }
  ...
  evaluate(req: ApprovalRequest): ApprovalDecision {
    ...
    for (const rule of this._denyRules) {
      if (rule.test(subject)) {
        return { approved: false, policy: this.policy, matchedRule: rule.source, reason: `Blocked by deny rule: ${rule.source}` };
      }
    }

    for (const rule of this._allowRules) {
      if (rule.test(subject)) {
        return { approved: true, policy: this.policy, matchedRule: rule.source, reason: `Permitted by allow rule: ${rule.source}` };
      }
    }
    ...
    switch (this.policy) {
      ...
      case 'on-request': {
        ...
        return { approved: isLow, policy: this.policy, reason: isLow ? 'Low-risk auto-approved' : `${req.riskLevel} risk requires approval` };
      }
    }
  }
  ...
  toJSON(): ApprovalEngineSnapshot {
    ...
    return {
      policy: this.policy,
      allowPatterns: this._allowRules.filter(r => !defaultSources.has(r.source)).map(r => r.source),
