...
export class AutomationPanelProvider implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage(async (message: { type: string; id?: string }) => {
      ...
      } else if (message.type === "stop_automation" && message.id) {
        await this.stopAutomation(message.id);
      } else if (message.type === "view_logs" && message.id) {
        await this.viewLogs(message.id);
      }
    });
