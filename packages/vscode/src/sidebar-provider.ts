  ...
  private async runAscendLoop(args: string): Promise<void> {
    ...
    try {
      while (this.ascendActive && this.ascendCycle < maxCycles) {
        ...
        // 4. Auto-commit so the SHA-based score sees changes. Capture the diffstat
        // so we can show the user what actually landed — separates "real edits
        // happened, score structurally stuck" from "model only narrated."
        try {
          ...
          if (status.trim().length > 0) {
            ...
            // Count source file lines changed BEFORE commit (only files under packages/, excluding state/build artifacts).
            ...
            try {
              ...
              for (const line of diffStat.split("\n")) {
                ...
                if (m && m[3] && m[3].startsWith("packages/") && !m[3].includes(".danteforge")) {
                  sourceLinesAdded += parseInt(m[1] ?? "0", 10);
                  sourceFilesChanged++;
                }
              }
              for (const f of untracked.split("\n").filter((l) => l.trim())) {
                if (f.startsWith("packages/") && !f.includes(".danteforge")) {
                  ...
                  try {
                    const wc = await runShellStreaming(
                      `wc -l "${f}"`,
                      { cwd: projectRoot, timeoutMs: 5_000 },
                    );
                    const n = parseInt(wc.trim().split(/\s+/)[0] ?? "0", 10);
                    if (Number.isFinite(n)) sourceLinesAdded += n;
                  } catch { /* ignore */ }
                }
              }
            } catch { /* ignore diffstat errors — commit still proceeds */ }
            ...
            commitsMade++;
            totalLinesChanged += sourceLinesAdded;
            const summary = sourceFilesChanged > 0
              ? `\n_Committed cycle ${this.ascendCycle}: **${sourceFilesChanged} source file${sourceFilesChanged === 1 ? "" : "s"}**, ~${sourceLinesAdded} lines._\n`
              : `\n_Committed cycle ${this.ascendCycle} (state files only — no source changes)._\n`;
            this.postMessage({
              type: "chat_response_chunk",
              payload: { chunk: summary, partial: "" },
            });
            this.postMessage({ type: "chat_response_done", payload: {} });
          }
        } catch (commitErr) {
        ...
        if (delta >= 0.1) {
          ...
          // Early-exit: 3 consecutive no-movement cycles. The action depends on
          // whether real edits actually committed during those cycles:
          // - commits == 0 → model isn't editing (Grok pattern). Halt + suggest model switch.
          // - commits > 0 → model IS editing but the harsh-scorer can't see the change
          //   on this codebase size. Halt + suggest test-on-smaller-project.
            ...
            if (commitsMade === 0) {
              note(
                `\n## 🛑 Loop halted — model is not making real edits\n\n` +
                  ...
                  (this.currentModel.toLowerCase().includes("grok")
                    ? `Grok-non-reasoning models often do Read/Glob calls and write analysis paragraphs but never execute Edit or Write. ` +
      ...
      const noScoreMovement = cyclesWithMovement === 0 && this.ascendCycle >= 2;
      const noRealEdits = commitsMade === 0 && this.ascendCycle >= 2;

      // Differentiate three failure modes for the closing tip:
      //   noRealEdits:    no commits at all. Model is narrating, not editing.
      //   noScoreMovement + commits > 0: edits landed but harsh-scorer didn't see them (codebase math).
      //   movement > 0:   actual progress. No tip needed.
      let tip = "";
      if (noRealEdits) {
        tip =
          `\n\n## ⚠️ Zero commits across ${this.ascendCycle} cycles\n\n` +
          `**Current model:** \`${this.currentModel}\`\n\n` +
          (isGrok
            ...
            : `The model isn't following through from analysis to actual edits.`);
      } else if (noScoreMovement) {
        tip =
          `\n\n## ℹ️ Real edits landed, score didn't move\n\n` +
          `**Commits this run:** ${commitsMade} (${totalLinesChanged} source lines added/changed)\n\n` +
