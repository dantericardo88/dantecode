...
const makeSkill = (overrides: Partial<Skill> & Pick<Skill, "id" | "title" | "content" | "section">): Skill => ({
...
describe("getRelevantSkills", () => {
  const skills: Skill[] = [
    makeSkill({ id: "s1", title: "TypeScript null checks", content: "Always add null checks", section: "coding" }),
    makeSkill({ id: "s2", title: "Async patterns", content: "Use async/await", section: "coding" }),
    makeSkill({ id: "s3", title: "Research strategies", content: "Use multiple sources", section: "research" }),
  ];
...
describe("scoreSkill", () => {
  it("scores higher with matching keywords", () => {
    const skill = makeSkill({ id: "s1", title: "TypeScript types", content: "Always use strict types", section: "coding" });
    const score = scoreSkill(skill, { taskType: "coding" }, ["typescript", "types"]);
    expect(score).toBeGreaterThan(0);
  });

  it("trust score boosts result", () => {
    const low = makeSkill({ id: "s1", title: "X", content: "do X", section: "coding", trustScore: 0.1 });
    const high = makeSkill({ id: "s2", title: "X", content: "do X", section: "coding", trustScore: 1.0 });
