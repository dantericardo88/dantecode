...
// src/verification-samples.ts
var GOOD_CODE_SAMPLE = `
...
`;
function runVerificationSamples() {
  const samples = [
...
function getExpectedSampleScores() {
  return [
...
// src/index.ts
function createStubPattern(regex, message, violationType, excludeExtensions) {
...
async function runAutoforgeIAL(code, context, config, router, projectRoot, onProgress) {
  ...
  for (let iteration = 1; iteration <= maxIterations; iteration++) {
    ...
    const failureSummary = gstackResults.filter((r) => !r.passed).map((r) => r.command).join(",");
    ...
    if (strategyMode === "reduced_scope") {
      prompt = `[STRATEGY: REDUCED SCOPE] Previous attempts failed with identical errors. Focus only on the single most critical failing check. Do not attempt to fix everything at once.

` + prompt;
    } else if (strategyMode === "minimal") {
      prompt = `[STRATEGY: MINIMAL REPRODUCTION] Multiple recovery attempts failed. Produce the smallest possible change that could fix the top-priority error. Ignore secondary failures for now.
...
` + prompt;
    ...
    try {
      ...
      const adaptiveMaxTokens = iterationFraction < 0.3 ? 8192 : iterationFraction < 0.6 ? 6144 : iterationFraction < 0.9 ? 4096 : 2048;
