...
interface DurableRunExecutionPolicyEvidence {
  filesCreated: string[];
...
interface DurableRunExecutionPolicyPhaseStatus {
  name: string;
  status: "pending" | "active" | "complete" | "failed";
  evidence?: DurableRunExecutionPolicyEvidence;
  verifiedAt?: number;
  error?: string;
}

interface DurableRunExecutionPolicyRetryEntry {
  tool: string;
  args: string;
  error?: string;
  timestamp: number;
...
interface DurableRunExecutionPolicyState {
  retryHistory?: DurableRunExecutionPolicyRetryEntry[];
  phases?: DurableRunExecutionPolicyPhaseStatus[];
}

type StoredDurableRun = DurableRun & {
  executionPolicyState?: DurableRunExecutionPolicyState;
};
...
export class DurableRunStore {
  ...
  async initializeRun(options: InitializeDurableRunOptions): Promise<StoredDurableRun> {
    const now = new Date().toISOString();
    const runId = options.runId ?? `run-${options.session.id}-${Date.now()}`;
    const run: StoredDurableRun = {
      id: runId,
  ...
  async checkpoint(runId: string, payload: DurableRunCheckpointPayload): Promise<StoredDurableRun> {
  ...
  async completeRun(runId: string, payload: DurableRunCheckpointPayload = {}): Promise<StoredDurableRun> {
    const current = await this.requireRun(runId);
    const evidence = await this.mergeEvidence(runId, payload.evidence ?? []);
    const next = this.mergeRun(current, {
      ...payload,
  ...
  async failRun(runId: string, payload: DurableRunCheckpointPayload = {}): Promise<StoredDurableRun> {
    const current = await this.requireRun(runId);
    const evidence = await this.mergeEvidence(runId, payload.evidence ?? []);
    const next = this.mergeRun(current, {
      ...payload,
  ...
  async loadRun(runId: string): Promise<StoredDurableRun | null> {
    return this.readJson<StoredDurableRun>(this.getRunFilePath(runId));
  }
  ...
  async listRuns(): Promise<StoredDurableRun[]> {
    const runs: StoredDurableRun[] = [];
  ...
  async getLatestWaitingUserRun(): Promise<StoredDurableRun | null> {
    const runs = await this.listRuns();
    return runs.find((run) => run.status === "waiting_user") ?? null;
  }

  private async listLegacyAutoforgeRuns(): Promise<StoredDurableRun[]> {
    const checkpointDir = join(this.projectRoot, ".dantecode", "autoforge-checkpoints");
    try {
      const entries = await readdir(checkpointDir);
      const rawRuns: Array<StoredDurableRun | null> = await Promise.all(
        entries
      ...
      return rawRuns.filter((run): run is StoredDurableRun => run !== null);
    } catch {
      return [];
    }
  }

  private async listLegacyBackgroundRuns(): Promise<StoredDurableRun[]> {
    const store = new BackgroundTaskStore(this.projectRoot);
    const tasks = await store.listTasks();
    return tasks.map((task) => this.backgroundTaskToRun(task));
  }

  private backgroundTaskToRun(task: BackgroundAgentTask): StoredDurableRun {
    ...
    await checkpointer.put(
      {
        ...
        executionPolicyState: run.executionPolicyState,
      },
      metadata,
      undefined,
      {
  ...
  private mergeRun(
    ...
    return {
      ...
      executionPolicyState: payload.executionPolicyState ?? current.executionPolicyState,
      resumeHint,
      updatedAt,
    };
  }
  ...
  private async requireRun(runId: string): Promise<StoredDurableRun> {
