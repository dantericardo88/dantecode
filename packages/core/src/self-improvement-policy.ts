...
export function isRepoInternalCdChain(command: string, projectRoot: string): boolean {
  ...
  // Check if destination is INSIDE the project root
  const isInternalPath =
    resolvedDestination === rootPath ||
    resolvedDestination.startsWith(`${rootPath}${sep}`);

  // FIXED LOGIC: Block external paths, allow internal paths
  // Return true = block, false = allow
  // Before: returned true for internal paths (BUG)
  // After: return true only for external paths
