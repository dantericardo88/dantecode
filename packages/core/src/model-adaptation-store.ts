...
import type {
  QuirkKey, QuirkObservation, CandidateOverride, OverrideStatus,
  ExperimentResult, ModelAdaptationSnapshot,
...
export type { QuirkObservation, CandidateOverride, ModelAdaptationSnapshot } from "./model-adaptation-types.js";
...
interface LegacyObservation {
  quirkClass: string; description: string; evidence: string;
  observedAt: string; sessionId: string;
}
interface LegacyOverride {
  id: string; key: ModelAdaptationKey; quirkClass: string; quirkSignature: string;
  overrideType: string; payload: string; version: number; evidenceCount: number;
  status: string; createdAt: string; updatedAt: string;
  promotionEvidence?: { testsPass: boolean; smokePass: boolean; pdseScore?: number };
}
interface LegacySnapshot { observations: LegacyObservation[]; overrides: LegacyOverride[]; version: number; }
...
function migrateObservation(old: LegacyObservation): QuirkObservation {
  ...
  return {
    id: generateId("obs"), quirkKey, provider: "legacy", model: "legacy", workflow: "other",
    promptTemplateVersion: "legacy", failureTags: [old.quirkClass],
    outputCharacteristics: [], evidenceRefs: [old.evidence], createdAt: old.observedAt,
  };
}
...
function migrateOverride(old: LegacyOverride): CandidateOverride {
  ...
  const validStatus = ["draft", "testing", "awaiting_review", "promoted", "rejected", "rolled_back"].includes(s) ? s : "draft" as const;
  return {
    id: old.id, provider: old.key.provider, model: old.key.modelId, quirkKey, status: validStatus,
    scope: { workflow: old.key.workflowType, commandName: old.key.commandType },
    patch: { promptPreamble: old.payload }, basedOnObservationIds: [], version: old.version,
...
function legacyObsToV2(old: LegacyObservation): Omit<QuirkObservation, "id" | "createdAt"> {
  const quirkKey = migrateLegacyQuirkClass(old.quirkClass) ?? "provider_specific_dispatch_shape";
  return {
    quirkKey, provider: "", model: "", workflow: "other", promptTemplateVersion: "legacy",
    failureTags: [old.quirkClass], outputCharacteristics: [], evidenceRefs: [old.evidence],
  };
}

function isLegacyDraft(obj: Record<string, unknown>): boolean {
  return typeof obj.quirkClass === "string" && typeof obj.payload === "string" &&
    obj.key !== undefined && typeof (obj.key as Record<string, unknown>).provider === "string";
}

function legacyDraftToV2(old: Record<string, unknown>): Omit<CandidateOverride, "id" | "version" | "status" | "createdAt"> {
  const key = old.key as ModelAdaptationKey;
  const quirkKey = migrateLegacyQuirkClass(old.quirkClass as string) ?? "provider_specific_dispatch_shape";
  return {
    provider: key.provider, model: key.modelId, quirkKey,
    scope: { workflow: key.workflowType, commandName: key.commandType },
    patch: { promptPreamble: old.payload as string }, basedOnObservationIds: [],
  };
}
...
export class ModelAdaptationStore {
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      if (parsed.version === 2) {
        ...
        this.observations = Array.isArray(legacy.observations) ? legacy.observations.map(migrateObservation) : [];
        this.overrides = Array.isArray(legacy.overrides) ? legacy.overrides.map(migrateOverride) : [];
        this.experiments = [];
      }
    } catch { this.observations = []; this.overrides = []; this.experiments = []; }
    this.loaded = true;
  }
  ...
  private async _doSave(rateLimiter?: ExperimentRateLimiter): Promise<void> {
    try {
      ...
      const snap: ModelAdaptationSnapshot & { rateLimiterState?: Record<string, { date: string; count: number }> } = {
        version: 2,
  ...
  addObservation(obs: any): QuirkObservation {
    ...
    const v2 = isLegacyObs(raw) ? legacyObsToV2(raw as unknown as LegacyObservation)
      : (obs as Omit<QuirkObservation, "id" | "createdAt">);
    const observation: QuirkObservation = { ...v2, id: generateId("obs"), createdAt: new Date().toISOString() };
  ...
  countObservations(quirkKey: string, providerOrKey: string | ModelAdaptationKey, model?: string): number {
    const [prov, mod] = typeof providerOrKey === "object"
    ...
    return this.observations.filter((o) => {
      ...
      const matchesQuirk = obs.quirkKey === quirkKey ||
        (Array.isArray(o.failureTags) && o.failureTags.includes(quirkKey));
      if (!matchesQuirk) return false;
      if (o.provider === "" && o.model === "") {
        return Array.isArray(o.evidenceRefs) && o.evidenceRefs.some((r) => r.includes(prov) && r.includes(mod));
      }
  ...
  getOverrides(providerOrKey: string | ModelAdaptationKey, modelOrStatus?: string | OverrideStatus, status?: OverrideStatus): CandidateOverride[] {
    let prov: string, mod: string, filterStatus: OverrideStatus | undefined;
    if (typeof providerOrKey === "object") {
      prov = providerOrKey.provider; mod = providerOrKey.modelId;
      filterStatus = modelOrStatus as OverrideStatus | undefined;
    } else {
      prov = providerOrKey; mod = modelOrStatus as string; filterStatus = status;
    }
  ...
  getActiveOverrides(providerOrKey: string | ModelAdaptationKey, model?: string): CandidateOverride[] {
  ...
  addDraft(draft: D12ADraftInput | LegacyDraftInput | Record<string, unknown>): CandidateOverride {
    ...
    const v2 = legacy ? legacyDraftToV2(draft)
      : (draft as Omit<CandidateOverride, "id" | "version" | "status" | "createdAt">);
    const override = {
      ...(legacy ? draft : {}), ...v2,
      id: generateId("ovr"), version: 1, status: "draft" as const, createdAt: now,
  ...
  rollbackOverride(overrideId: string): CandidateOverride | null {
    ...
    const rollback: CandidateOverride = {
      id: generateId("ovr"), provider: original.provider, model: original.model,
      quirkKey: original.quirkKey, status: "draft", scope: { ...original.scope },
      patch: { ...original.patch }, basedOnObservationIds: [...original.basedOnObservationIds],
      version: original.version + 1, createdAt: new Date().toISOString(),
  ...
  addExperiment(result: ExperimentResult): void { this.experiments.push(result); }
  ...
  snapshot(): ModelAdaptationSnapshot {
    return {
      version: 2, observations: [...this.observations],
      overrides: [...this.overrides], experiments: [...this.experiments],
    };
  }
