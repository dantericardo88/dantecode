...
/**
 * Options for extracting claimed files.
 */
export interface ExtractClaimedFilesOptions {
  /**
   * If true, only match past-tense action verbs (e.g., "Created", "Modified").
   * If false (default), match any file mention including future tense ("will edit").
   * This helps distinguish actual completion claims from planning/intent.
   */
  actionVerbsOnly?: boolean;
}
 ...
 *
 * Enhanced with action-verb filtering to reduce false positives from planning text.
 */
export function extractClaimedFiles(
  text: string,
  options?: ExtractClaimedFilesOptions,
): string[] {
  const files = new Set<string>();

  if (options?.actionVerbsOnly) {
    // Strict mode: Only match PAST-TENSE action verbs (actual completion claims)
    // Excludes: "will edit", "going to modify", "should update"
    const pastTensePatterns = [
      // Past tense: Created, Modified, Updated, etc.
      /(?:Created|Modified|Updated|Fixed|Wrote|Edited|Added|Removed|Deleted)\s+[`"']?([^\s`"',]+\.\w{1,6})/gi,
      // Tool name as past action: "Wrote to", "Edited"
    ...
    // Permissive mode: Match any file mentions (original behavior)
    const allPatterns = [
      /(?:updated|modified|edited|wrote|created|changed|will\s+edit|will\s+modify|should\s+update)\s+[`"']?([^\s`"',]+\.\w{1,6})/gi,
      /(?:Write|Edit)\s+(?:to\s+)?[`"']?([^\s`"',]+\.\w{1,6})/g,
    ];
