...
export interface ReplOptions {
  ...
  yolo?: boolean;
  /** --execution-profile: explicit runtime profile for deterministic automation paths. */
  executionProfile?: ExecutionProfile;
}

export type ExecutionProfile = "default" | "benchmark";
...
export async function startRepl(options: ReplOptions): Promise<void> {
  ...
  const replState: ReplState = {
    ...
    approvalMode: options.yolo || options.executionProfile === "benchmark" ? "yolo" : "review",
    taskMode: null,
...
export async function runOneShotPrompt(prompt: string, options: ReplOptions): Promise<void> {
  ...
  const agentConfig: AgentLoopConfig = {
    ...
    silent: options.silent,
    fearSetBlockOnNoGo:
      options.executionProfile === "benchmark" ? false : options.fearSetBlockOnNoGo === true,
    confidenceGating:
  ...
  if (options.yolo || options.executionProfile === "benchmark") {
