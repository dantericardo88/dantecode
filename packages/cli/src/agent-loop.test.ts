...
vi.mock("@dantecode/core", async () => {
  ...
  return {
    ...
    // getGlobalTraceLogger — returns trace logger for observability
    ...
    // BoundaryTracker — tracks scope drift across tool rounds
    ...
    // calculatePressure — pressure metrics for auto-thinking
...
describe("runAgentLoop smoke tests", () => {
  ...
  it("nudges narrated execution into real tool calls for action prompts", async () => {
    ...
    const session = makeSession();
    await runAgentLoop("Fix src/index.ts", session, makeConfig());
...
describe("Pipeline continuation nudge", () => {
  ...
  it("stops nudging after MAX_PIPELINE_CONTINUATION_NUDGES (3)", async () => {
    ...
    expect(mockGenerateText.mock.calls.length).toBeLessThanOrEqual(9);
  });
});
...
describe("Dynamic round budget (requiredRounds)", () => {
  ...
  it("uses requiredRounds when provided", async () => {
    ...
    // Use unique Edit targets so the fail-closed workflow engine sees real file mutations
    // instead of a reads-only transcript that should now trigger confab blocking.
    for (let i = 0; i < 17; i++) {
      const toolCallText = `<tool_use>\n{"name":"Edit","input":{"file_path":"file-${i}.ts","old_string":"alpha-${i}","new_string":"beta-${i}"}}\n</tool_use>`;
      mockGenerateText.mockResolvedValueOnce({
        text: `Round ${i + 1}.\n${toolCallText}`,
        usage: { totalTokens: 50 },
      });
    ...
    // Final response intentionally avoids completion-style wording so this test
    // measures round budget only, not completion nudges.
    mockGenerateText.mockResolvedValueOnce({
      text: "Round budget satisfied. Awaiting follow-up instructions.",
      usage: { totalTokens: 20 },
    });
    ...
    // All 17 tool rounds should have executed (impossible with the default maxToolRounds=15).
    expect(mockExecuteTool.mock.calls.length).toBeGreaterThanOrEqual(17);
    expect(mockGenerateText.mock.calls.length).toBeGreaterThanOrEqual(18);
  });

  it("enforces minimum of 15 rounds even when requiredRounds is lower", async () => {
    // Even if requiredRounds is 5, the loop should still allow the 15-round floor.
    for (let i = 0; i < 15; i++) {
      const toolCallText = `<tool_use>\n{"name":"Edit","input":{"file_path":"floor-${i}.ts","old_string":"old-${i}","new_string":"new-${i}"}}\n</tool_use>`;
      mockGenerateText.mockResolvedValueOnce({
        text: `Floor round ${i + 1}.\n${toolCallText}`,
        usage: { totalTokens: 40 },
      });
      mockExecuteTool.mockResolvedValueOnce({ content: `floor ${i + 1}`, isError: false });
    }
    mockGenerateText.mockResolvedValueOnce({
      text: "Minimum round budget reached. Awaiting follow-up instructions.",
      usage: { totalTokens: 20 },
    });
    ...
    // The floor should still allow 15 executed rounds even when requiredRounds is lower.
    expect(mockExecuteTool.mock.calls.length).toBeGreaterThanOrEqual(15);
    expect(mockGenerateText.mock.calls.length).toBeGreaterThanOrEqual(16);
  });
});
...
describe("Universal skill completion (skillActive)", () => {
  ...
  it("resets empty round counter when tool calls succeed", async () => {
    ...
    const session = makeSession();
    const result = await runAgentLoop("Read src/app.ts", session, makeConfig());
