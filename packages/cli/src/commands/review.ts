...
async function getGitHubClient(projectRoot: string, token?: string): Promise<GitHubClient> {
...
export async function reviewPR(
  ...
  for (const file of reviewableFiles) {
    ...
    if (!passed) {
      ...
      const desc = findings.find((f) => f.includes("FAILED")) ?? "DanteForge quality gate failed";
      bugs.push({
        file: file.filename,
        line,
        description: desc,
  ...
  const overallScore =
    fileReviews.length > 0
      ? Math.round(fileReviews.reduce((s, f) => s + f.pdseScore, 0) / fileReviews.length)
      : 100;

  // 4. Determine recommendation (severity-aware thresholds)
  const approveThreshold =
    options.severity === "strict" ? 90 : options.severity === "lenient" ? 70 : 80;
  const commentThreshold =
    options.severity === "strict" ? 70 : options.severity === "lenient" ? 50 : 60;
  ...
  // 5. Build summary
  const summary = formatReviewSummary(prNumber, fileReviews, bugs, overallScore, recommendation);
...
export function formatReviewSummary(
  ...
  if (bugs.length > 0) {
    ...
    for (const b of bugs) {
      lines.push(`- **${b.severity.toUpperCase()}** \`${b.file}:${b.line}\` — ${b.description}`);
    }
    lines.push(``);
  }
  ...
  lines.push(`---`);
  lines.push(
    `*Generated by DanteCode DanteReview — DanteForge PDSE + Constitutional Verification*`,
  );
...
export function formatReviewOutput(result: ReviewResult): string {
  ...
  if (result.fileReviews.length > 0) {
    ...
    for (const fr of result.fileReviews) {
      ...
      const extras = fr.stubViolation
        ? ` ${RED}[STUB]${RESET}`
        : fr.constitutionViolation
          ? ` ${RED}[CONST]${RESET}`
          : "";
...
export async function runReviewCommand(args: string[], projectRoot: string): Promise<void> {
  const sub = args[0];
